...
void apply_boundary_conditions() {
    ...
    for (int i = 0; i <= nx; i++) {
        u[ny-1][i] = 3.0; // Lid moves to the right
    }
    for (int i = 0; i < nx; i++) {
        v[ny][i] = 0.0;
    }
    ...
    // Left wall (AB): no-slip, u = 0, v = 0 (with slip condition for v)
    for (int j = 0; j < ny; j++) {
        u[j][0] = 0.0;
        if (j > 0 && j < ny) {
            v[j][0] = v[j][1]; // Slip condition: zero gradient for v
        }
    }

    // Right wall (CD): no-slip, u = 0, v = 0 (with slip condition for v)
    for (int j = 0; j < ny; j++) {
        u[j][nx] = 0.0;
        if (j > 0 && j < ny) {
            v[j][nx-1] = v[j][nx-2]; // Slip condition: zero gradient for v
        }
    }

    // Corners: ensure u = v = 0
    u[0][0] = 0.0; u[0][nx] = 0.0; u[ny-1][0] = 0.0; u[ny-1][nx] = 0.0;
    v[0][0] = 0.0; v[0][nx-1] = 0.0; v[ny][0] = 0.0; v[ny][nx-1] = 0.0;
